...
Write-Host ""
       ...
       if ($Method -eq "direct") {
           $sshArgs = "-p 19936 root@175.155.64.235 -L 5678:localhost:5678 -L 8501:localhost:8501 -L 8001:localhost:8001 -L 8002:localhost:8002 -L 8003:localhost:8003 -L 8004:localhost:8004 -L 11434:localhost:11434"
       } else {
           $sshArgs = "-p 10873 root@ssh4.vast.ai -L 5678:localhost:5678 -L 8501:localhost:8501 -L 8001:localhost:8001 -L 8002:localhost:8002 -L 8003:localhost:8003 -L 8004:localhost:8004 -L 11434:localhost:11434"
       }

# Test connection first (but don't fail - just warn)
Write-Host "Testing SSH connection..." -ForegroundColor Yellow
if ($Method -eq "direct") {
    $testCmd = "ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -p 19936 root@175.155.64.235 'echo test' 2>&1"
} else {
    $testCmd = "ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -p 10873 root@ssh4.vast.ai 'echo test' 2>&1"
}
